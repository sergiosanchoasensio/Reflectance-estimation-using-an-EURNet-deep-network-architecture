...
def random_crop(x, y, random_crop_size, sync_seed=None, **kwargs):
...
class ImageDataGenerator(object):
    ...
    def flow(self, X, y=None, batch_size=32, shuffle=True, seed=None,
        ...
        # barrel/fisheye --alt shift change keyboard language
    ...
    def random_transform(self, x, y):
        ...
        rangew = (cfg.shape_input_data[1] - cfg.shape_train_data[1]) // 2
        rangeh = (cfg.shape_input_data[2] - cfg.shape_train_data[2]) // 2
        random_value = np.random.random()
        # Crop
        if random_value < 0.5:
            offsetw = 0 if rangew == 0 else np.random.randint(rangew)
            offseth = 0 if rangeh == 0 else np.random.randint(rangeh)
            x = x[:, offsetw:offsetw+cfg.shape_train_data[1], offseth:offseth+cfg.shape_train_data[2]]
            y = y[:, offsetw:offsetw+cfg.shape_train_data[1], offseth:offseth+cfg.shape_train_data[2]]
        # Entire image
        else:
            # Clean image
            current_image_channel = x[0]
            m_r = cv2.resize(current_image_channel, (cfg.shape_train_data[2], cfg.shape_train_data[1]))
            current_image_channel = x[1]
            m_g = cv2.resize(current_image_channel, (cfg.shape_train_data[2], cfg.shape_train_data[1]))
            current_image_channel = x[2]
            m_b = cv2.resize(current_image_channel, (cfg.shape_train_data[2], cfg.shape_train_data[1]))
            # Albedo image
            current_image_channel = y[0]
            n_r = cv2.resize(current_image_channel, (cfg.shape_train_data[2], cfg.shape_train_data[1]))
            current_image_channel = y[1]
            n_g = cv2.resize(current_image_channel, (cfg.shape_train_data[2], cfg.shape_train_data[1]))
            current_image_channel = y[2]
            n_b = cv2.resize(current_image_channel, (cfg.shape_train_data[2], cfg.shape_train_data[1]))
            x = np.stack((m_r, m_g, m_b))
            y = np.stack((n_r, n_g, n_b))
...
class NumpyArrayIterator(Iterator):
    ...
    def next(self):
        ...
        # Normal model
        train_shape = (current_batch_size, cfg.shape_train_data[0], cfg.shape_train_data[1], cfg.shape_train_data[2])
        batch_x = np.zeros(tuple([current_batch_size] + list(train_shape)[1:]))
        batch_y = np.zeros(tuple([current_batch_size] + list(train_shape)[1:]))
        
        # With 2 head model     
        '''
        batch_x = [None] * current_batch_size
        ...
        for i, j in enumerate(index_array):
            ...
            #x = self.image_data_generator.standardize(x) # Les x resultants son negres!
            ...
            # With 2 head model
            '''
            batch_x[i] = {'main_input': x, 'aux_input': x}
            batch_y[i] = {'main_output': label, 'aux_output': label}
            '''
            
            # save images
            '''
            cfg.aux = cfg.aux + 1 
            cv2.imwrite('DA/'+str(cfg.aux)+'_x.jpg', x.transpose(1,2,0) * 255.0)
            cv2.imwrite('DA/'+str(cfg.aux)+'_y.jpg', label.transpose(1,2,0) * 255.0)
            '''
        
        #return batch_x, batch_y
        return {'main_input': batch_x}, {'main_output': batch_y, 'aux_output': batch_y}